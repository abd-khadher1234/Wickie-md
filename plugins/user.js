...
const isBotAdmins=async a=>{var i=a.isGroup?await a.client.groupMetadata(a.chat).catch(a=>{}):"";const t=a.isGroup?await i.participants:"",s=a.isGroup?await t.filter(a=>null!==a.admin).map(a=>a.id):"";return!!a.isGroup&&s.includes(a.user_id)};
Function({pattern: 'pp ?(.*)', fromMe: true, desc: 'set profile picture in any resolution', type: 'user'}, async (message, match) => {
...
Function({pattern: 'fullpp ?(.*)', fromMe: true, desc: 'set profile picture in any resolution', type: 'user'}, async (message, match) => {
if (!message.reply_message || !message.reply_message.image) return await message.send('_Reply to a image._')
const media = await message.reply_message.downloadAndSaveMedia()
await message.updateProfilePicture(message.user_id, media)
await message.send('_Successfully Profile Picture Updated_')
...
Function({pattern: 'gpp ?(.*)', fromMe: true, desc: 'set group icon in any resolution', type: 'group'}, async (message, match) => {
...
Function({pattern: 'block', fromMe: true, desc: 'Block a person', type: 'user'}, async (message, match) => {
...
Function({pattern: 'unblock', fromMe: true, desc: 'Unblock a person', type: 'user'}, async (message, match) => {
const id = message.mention[0] || message.reply_message.sender || (!message.isGroup && message.jid)
if (!id) return await message.send('*Give me a user*')
