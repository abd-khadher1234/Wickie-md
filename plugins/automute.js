const { Function, prefix, setSchedule, getSchedule, getAllSchedule, addSchedule } = require('../lib/')
...
Function({pattern: 'automute ?(.*)', fromMe: true, desc: 'auto group mute scheduler', type: 'group'}, async (message, match, client) => {
...
buttons: [{buttonId: prefix + 'automute on', buttonText: {displayText: 'ON'}, type: 1},{buttonId: prefix + 'automute off', buttonText: {displayText: 'OFF'}, type: 1},{buttonId: prefix + 'automute get', buttonText: {displayText: 'GET'}, type: 1}],
headerType: 1
}
...
Function({pattern: 'autounmute ?(.*)', fromMe: true, desc: 'auto group unmute scheduler', type: 'group'}, async (message, match, client) => {
if (!message.isGroup) return await message.reply('_This command only works in group chats_')
const isbotAdmin = await isBotAdmins(message)
if (!isbotAdmin) return await message.reply("I'm not an admin")
const groupMetadata = await message.client.groupMetadata(message.jid)
